...
export default function startApp(...modules: AsyncModuleMap[]) {
  ...
  const allModules: AsyncModuleMap = Object.assign({}, EXPORTED_MODULES, ...modules)
...
const createAsyncControllerResolver = (pathMap: AsyncModuleMap) => {
  ...
  return async (key: string) => {
    ...
    if (!module) {
      throw Error(`Controller not found: ${key}. Have you named the file ${key}-controller.ts?`)
    }

    if (!module.default || !(module.default.prototype instanceof Controller)) {
      throw Error(`Module ${key} should have as its default export a subclass of Controller`)
    }
 ...
 **/
const getIdentifierFromPath = (fullPath: string) =>/([a-zA-Z-_0-9]*)[-_]controller\.(t|j)sx?$/.exec(fullPath)?.[1]
